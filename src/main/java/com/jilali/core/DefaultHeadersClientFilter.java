...
public class DefaultHeadersClientFilter implements Ordered {
    ...
    private static final Logger log = LoggerFactory.getLogger(DefaultHeadersClientFilter.class);
    ...
    public void addDefaults(MutableHttpRequest<?> downstream) {
        ...
        downstream.header("x-b3-spanid", "%016x".formatted(System.nanoTime() & 0xFFFF_FFFF_FFFFL));
        downstream.header("x-request-start", String.valueOf(System.currentTimeMillis()));
    }

    private String defaultFor(String header) {
        return switch (header) {
            case "authorization", "x-ht-token" -> "Bearer " + properties.defaultAuthToken();
            case "user-agent" -> "%s;%s;%s;%s;".formatted("ios", "6.1.0", "iPhone", "18.5");
