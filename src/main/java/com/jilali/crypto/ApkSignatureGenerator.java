 ...
 * packet requires. Ported verbatim from the reference client (connectwebsock.js's
 * {@code _APK_SIG} object) - same HMAC key, same certificate blob, same field ordering.
 *
 * <p>Critically: the HMAC key is the literal HEX STRING below, UTF-8 encoded as-is - NOT
 * hex-decoded into 32 raw bytes first. The reference client's {@code _hmacHex} does
 * {@code new TextEncoder().encode(keyHex)}, encoding the 64-character string itself.
 */
public final class ApkSignatureGenerator {
     ...
     *  the reference client's {@code substring(0, 499)} / {@code substring(499, 998)}. */
