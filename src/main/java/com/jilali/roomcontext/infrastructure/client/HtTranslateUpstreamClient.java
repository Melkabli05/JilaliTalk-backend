...
/** Dedicated AI-translator upstream caller for this bounded context - posts directly to
 *  HelloTalk's {@code /translate/v2/ai_translator/translate} endpoint on the shared
 *  {@code @Client("jlhub")} HTTP client bean (same connection pool/config as every other
 *  dedicated client in this package, keyed by id not by interface - see
 *  docs/room-redesign/06-package-dependency-analysis.md). No dependency on the legacy
 *  translate.* feature package. */
@Singleton
public class HtTranslateUpstreamClient {

    private static final Logger log = LoggerFactory.getLogger(HtTranslateUpstreamClient.class);
    private static final String UPSTREAM_PATH = "/translate/v2/ai_translator/translate";

    private final HttpClient httpClient;
    private final ObjectMapper mapper;

    public HtTranslateUpstreamClient(@Client("jlhub") HttpClient httpClient, ObjectMapper mapper) {
